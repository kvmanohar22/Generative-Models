...
class Dataset(object):
	def __init__(self, opts):
		...
		if self.opts.dataset == 'CIFAR':
			imgs_file = self.opts.root_dir+self.opts.dataset_dir+'data_batch_2'
			...
			data_dir = self.opts.root_dir + self.opts.dataset_dir
	...
	def save_batch_images(self, images, grid, img_file, normalized=False):
		...
		imsave(self.opts.root_dir+self.opts.sample_dir+img_file, imgs)
